  ...
  },
  {
    ...
    response: (
      <div className="space-y-2 max-h-[300px] overflow-y-auto pr-2 custom-scrollbar">
        ...
        <div className="mt-2 pt-2 border-t border-gray-200 dark:border-white/10 text-xs text-maco-red font-medium">
    ...
    <AnimatePresence>
      {isOpen && (
        <motion.div
          ...
          className="fixed bottom-24 right-4 md:right-8 w-[90vw] md:w-[400px] h-[600px] max-h-[80vh] bg-white dark:bg-[#1a1c23] border border-gray-200 dark:border-white/10 rounded-2xl shadow-[0_20px_50px_-10px_rgba(0,0,0,0.3)] z-[100] flex flex-col overflow-hidden backdrop-blur-xl"
        >
          {/* Header */}
          <div className="bg-gradient-to-r from-gray-900 to-black dark:from-[#111316] dark:to-[#0a0a0c] px-5 py-4 flex justify-between items-center border-b border-gray-800 relative overflow-hidden">
          ...
          {/* Chat Area */}
          <div className="flex-1 overflow-y-auto p-4 space-y-5 bg-gray-50/50 dark:bg-[#111316]/50 scroll-smooth custom-scrollbar">
            {messages.map((msg) => (
              <motion.div
                key={msg.id}
              ...
              >
                {msg.type === 'bot' && (
                  <div className="w-7 h-7 rounded-full bg-gray-200 dark:bg-white/10 flex items-center justify-center flex-shrink-0 mb-1 shadow-sm">
                ...
                <div 
                  className={`max-w-[78%] px-4 py-3 rounded-2xl text-sm leading-relaxed shadow-sm ${
                    msg.type === 'user' 
                      ...
                      : 'bg-white dark:bg-[#1e2028] border border-gray-200 dark:border-white/5 text-gray-800 dark:text-gray-200 rounded-bl-sm shadow-black/5'
                  }`}
                >
            ...
            {isThinking && (
              ...
              >
                ...
                <div className="bg-white dark:bg-[#1e2028] border border-gray-200 dark:border-white/5 px-4 py-4 rounded-2xl rounded-bl-sm flex gap-1.5 shadow-sm">
          ...
          <div className="bg-white dark:bg-[#1a1c23] border-t border-gray-200 dark:border-white/10 flex flex-col relative z-20">
                ...
                >
                  {latestBotMessage.quickReplies.map((q, idx) => (
                    <motion.button
                      ...
                      disabled={isThinking}
                      className="inline-flex flex-shrink-0 items-center justify-center text-[12px] font-medium bg-maco-red/10 hover:bg-maco-red/20 dark:bg-white/5 dark:hover:bg-white/10 text-maco-red dark:text-gray-200 border border-maco-red/20 dark:border-white/10 px-3 py-1.5 rounded-full transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                      {q}
                    </motion.button>
                  ))}
            ...
            <div className="p-3">
              <form onSubmit={handleSend} className="relative flex items-center group">
                <input
                  ...
                  className="w-full bg-gray-100 dark:bg-[#111316] border border-transparent focus:border-maco-red/30 dark:focus:border-white/20 rounded-full pl-4 pr-12 py-3 text-sm text-gray-900 dark:text-white placeholder-gray-500 focus:outline-none transition-all duration-300 shadow-inner group-focus-within:bg-white dark:group-focus-within:bg-[#0a0a0c]"
                />
                <button
                  type="submit"
                  disabled={!inputValue.trim() || isThinking}
                  className="absolute right-1.5 p-2 bg-maco-red hover:bg-red-700 disabled:bg-gray-300 dark:disabled:bg-gray-800 disabled:text-gray-500 text-white rounded-full transition-all duration-300 flex items-center justify-center shadow-md shadow-maco-red/20 disabled:shadow-none"
                >
